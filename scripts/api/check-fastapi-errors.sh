...
echo ""
echo "2. Code Check:"
if ssh "$SERVER" "cd $APP_PATH && test -f src/main.py"; then
...
IMPORT_TEST=$(ssh "$SERVER" "cd $APP_PATH && source venv/bin/activate && python -c 'import src.main' 2>&1")
if [ $? -eq 0 ]; then
...
MANUAL_TEST=$(ssh "$SERVER" "cd $APP_PATH && source venv/bin/activate && timeout 3 python -m src.main 2>&1" | head -10)
