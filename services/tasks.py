...
def collect_option_chain_task(self, symbol: str) -> dict:
    ...
    try:
        ...
        logger.info(f"Completed option chain collection for {symbol}: {result.get('records_collected', 0)} records")
        return {
            'status': 'success',
            'symbol': symbol,
        ...
        logger.error(f"Failed to collect option chain for {symbol}: {exc}", exc_info=True)
        # Retry the task
