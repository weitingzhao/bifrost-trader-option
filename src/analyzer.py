...
from .models import (
    OptionsChain, StrategyResult, StrategyType,
    CoveredCallParams, IronCondorParams, OptionContract, OptionType
)
...
class StrategyAnalyzer:
    ...
    async def analyze_covered_call(
        ...
        call_contracts = [
            ...
            if c.option_type == OptionType.CALL and c.expiration == params.call_expiration
        ]
        
        if not call_contracts:
            logger.warning(f"No call contracts found for {params.symbol} expiring {params.call_expiration}")
            return results
        ...
        for call_contract in call_contracts:
            try:
                ...
                logger.error(f"Error analyzing covered call with strike {call_contract.strike}: {e}")
                continue
    ...
    async def analyze_iron_condor(
        ...
        contracts = [
            c for c in chain.contracts
            if c.expiration == params.expiration
        ]
        
        if not contracts:
            logger.warning(f"No contracts found for {params.symbol} expiring {params.expiration}")
            return results
        
        # Find specific contracts
        put_sell = next(
            (c for c in contracts 
             if c.option_type == OptionType.PUT and c.strike == params.put_sell_strike),
            None
        )
        put_buy = next(
            (c for c in contracts 
        ...
        call_sell = next(
            (c for c in contracts 
             if c.option_type == OptionType.CALL and c.strike == params.call_sell_strike),
            None
        )
        call_buy = next(
            (c for c in contracts 
             if c.option_type == OptionType.CALL and c.strike == params.call_buy_strike),
            None
        )
        
        if not all([put_sell, put_buy, call_sell, call_buy]):
            missing = []
            if not put_sell: missing.append(f"put_sell ({params.put_sell_strike})")
            if not put_buy: missing.append(f"put_buy ({params.put_buy_strike})")
            if not call_sell: missing.append(f"call_sell ({params.call_sell_strike})")
            if not call_buy: missing.append(f"call_buy ({params.call_buy_strike})")
    ...
    async def analyze_iron_condor_variations(
        ...
        puts = sorted([c for c in contracts if c.option_type == OptionType.PUT], key=lambda x: x.strike)
        calls = sorted([c for c in contracts if c.option_type == OptionType.CALL], key=lambda x: x.strike)
                ...
                for k in range(len(calls) - 1):
                    for l in range(k + 1, len(calls)):
                        ...
                        try:
                            ...
                            if entry_cost > -min_credit:  # entry_cost is negative for credit
                                continue
