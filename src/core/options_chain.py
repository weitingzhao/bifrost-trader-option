...
class OptionsChainFetcher:
    ...
    def __init__(self):
        ...
        self._cache: Dict[str, Tuple[OptionsChain, float]] = {}  # symbol -> (chain, timestamp)
    ...
    async def fetch_chain(self, symbol: str, use_cache: bool = True) -> Optional[OptionsChain]:
        """
        ...
        try:
            ...
            underlying_price = await connector.get_stock_price(symbol)
            if underlying_price is None:
                logger.warning(f"Could not get stock price for {symbol}")
                underlying_price = 0.0
            
            # Get option chain contract details
            contract_details_list = await connector.get_option_chain(symbol)
            ...
            for i in range(0, len(contract_details_list), batch_size):
                ...
                for details, ticker in zip(batch, tickers):
                    ...
                    mid = (bid + ask) / 2 if bid > 0 and ask > 0 else (ticker.close if ticker.close else 0.0)
                    ...
                    # Get IV
                    iv = ticker.modelGreeks.impliedVolatility if ticker.modelGreeks else None
                    ...
                    option_contract = OptionContract(
                        ...
                        open_interest=details.contract.openInterest if hasattr(details.contract, 'openInterest') else 0,
    ...
    async def _get_tickers_batch(self, contract_details_list: List) -> List[Optional[Ticker]]:
