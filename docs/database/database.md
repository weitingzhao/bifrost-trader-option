...
3. **`scripts/database/schema_*.sql`** (App-Specific Schema Files)
   - SQL representation organized by Django app
...
4. **`scripts/database/schema_all.sql`** (Combined Schema)
   - Auto-generated by `combine_schemas.sh`
   - Combines all app-specific schema files into one
   - Used for database setup and deployment
   - Also copied to `docs/database/schema_all.sql` for documentation

5. **`docs/database/schema_all.sql`** (Documentation Copy)
   - Copy of the combined schema for easy reference
   - Auto-updated when running `combine_schemas.sh`
   - Available in documentation for viewing complete schema

### Generated/Exported Files

6. **`scripts/database/schema_sqlalchemy.sql`**
...
7. **`scripts/database/schema_django.sql`**
