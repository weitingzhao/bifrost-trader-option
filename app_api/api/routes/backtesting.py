...
from app_api.database.repositories.history_repo import get_history_repository
...
async def run_backtest(
    ...
    try:
        ...
        if not strategy_type or not symbol:
            raise HTTPException(
                status_code=400, detail="strategy_type and symbol are required"
            )
        ...
        snapshots = await repo.get_option_snapshots(
            symbol=symbol, start_time=start, end_time=end, limit=1000
        )
        ...
        contracts_data = (
            first_snapshot.contracts_data if first_snapshot.contracts_data else []
        )
        ...
        if strategy_type == "covered_call":
            ...
            for contract in contracts_data:
                if (
                    contract.get("expiration") == params.call_expiration
                    and contract.get("strike") == params.call_strike
            ...
            for contract in contracts_data:
                ...
                if (
                    contract.get("option_type") == "PUT"
                    and contract.get("strike") == params.put_sell_strike
                ):
                    put_sell_data = contract
                elif (
                    contract.get("option_type") == "PUT"
                    and contract.get("strike") == params.put_buy_strike
                ):
                    put_buy_data = contract
                elif (
                    contract.get("option_type") == "CALL"
                    and contract.get("strike") == params.call_sell_strike
                ):
                    call_sell_data = contract
                elif (
                    contract.get("option_type") == "CALL"
                    and contract.get("strike") == params.call_buy_strike
                ):
            ...
            raise HTTPException(
                status_code=400, detail=f"Unknown strategy type: {strategy_type}"
            )
